...
deploy-rabbitmq: docker-rabbitmq
	...
	$(call generate-rabbitmq-svc) | kubectl apply -n $(NAMESPACE) -f -
	...
	$(call generate-rabbitmq-headless-svc) | kubectl apply -n $(NAMESPACE) -f -
	if [ "$(RABBITMQ_EXPOSE_MANAGEMENT)" = "TRUE" ]; then kubectl get svc -n $(NAMESPACE) $(RABBITMQ_MANAGEMENT_SERVICE_NAME) || $(call generate-rabbitmq-management-svc) | kubectl apply -n $(NAMESPACE) -f - ; fi
